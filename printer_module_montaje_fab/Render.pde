void renderThumbs() {
  ...
  for ( int i = 0; i < f.size (); i ++ ) {
    ...
    image(f.get(i).thumb, PADDING/2, PADDING/2 + 10);
...
void renderId(int i) {
  ...
  String name = "SUJETO NRO. " + nf(float(f.get(i).id), 4, 0);
...
void renderHighlight() {
  ...
  int alfa = int(map(millis()-select_trigger, 0, manual_select_timeout, 150, 20));
